...
function generateOsuHitStatistics(
  options: IHitStatisticsInput
): IHitStatistics {
...
function generateTaikoHitStatistics(
  options: IHitStatisticsInput
): IHitStatistics {
  const beatmap = options.beatmap;
  const accuracy = getAccuracy(options);
...
function generateCatchHitStatistics(
  options: IHitStatisticsInput
): IHitStatistics {
  const beatmap = options.beatmap;
  const accuracy = getAccuracy(options);
...
function generateManiaHitStatistics(
  options: IHitStatisticsInput
): IHitStatistics {
  ...
  if (typeof count50 === "number" || typeof options.accuracy !== "number") {
    count50 = count50
      ? MathUtils.clamp(count50, 0, totalHits - currentCounts)
      : 0;
  } else {
    /**
     * Acc = 0.98, Total = 1000
  ...
  const countGeki =
    totalHits - count300 - countKatu - count100 - count50 - countMiss;
